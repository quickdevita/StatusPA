...
document.getElementById("modal-container").addEventListener("click", function (event) {
 ...
 // Aggiungi la gestione del pulsante stella
 const favoriteButton = document.getElementById("favorite-button");
 checkIfFavorite(lavoroId, favoriteButton);

 document.getElementById("modal-container").classList.add("open");
 document.getElementById("modal").classList.add("open");
 document.body.classList.add("modal-open");

 // Gestione del clic sulla stella per aggiungere ai preferiti
 favoriteButton.addEventListener("click", async () => {
   toggleFavorite(lavoroId, favoriteButton);
 });


// Funzione per controllare se un lavoro è nei preferiti
async function checkIfFavorite(lavoroId, favoriteButton) {
 const preferiti = await caricaPreferiti();
 const isFavorite = preferiti.some(lavoro => lavoro.id === lavoroId);

 if (isFavorite) {
   favoriteButton.classList.add("favorito"); // Modifica l'aspetto della stella per i preferiti
 } else {
   favoriteButton.classList.remove("favorito");
 }
}

// Funzione per aggiungere o rimuovere un lavoro dai preferiti
async function toggleFavorite(lavoroId, favoriteButton) {
 const preferiti = await caricaPreferiti();
 const isFavorite = preferiti.some(lavoro => lavoro.id === lavoroId);

 if (isFavorite) {
   // Rimuovi il lavoro dai preferiti
   const updatedFavorites = preferiti.filter(lavoro => lavoro.id !== lavoroId);
   await salvaPreferiti(updatedFavorites);
   favoriteButton.classList.remove("favorito");
 } else {
   // Aggiungi il lavoro ai preferiti
   const lavoro = { id: lavoroId, title: document.getElementById("modal-title").textContent };
   preferiti.push(lavoro);
   await salvaPreferiti(preferiti);
   favoriteButton.classList.add("favorito");
 }
}

// Funzione per caricare i preferiti dalla cache
async function caricaPreferiti() {
 const cache = await caches.open('user-preferences');
 const response = await cache.match('preferiti');
 return response ? response.json() : [];
}

// Funzione per salvare i preferiti nella cache
async function salvaPreferiti(preferiti) {
 const cache = await caches.open('user-preferences');
 const response = new Response(JSON.stringify(preferiti), {
   headers: { 'Content-Type': 'application/json' },
 });
 await cache.put('preferiti', response);
}
...
// Aggiungi un lavoro ai preferiti
async function aggiungiPreferito(lavoro) {
  const cache = await caches.open('preferiti-cache');
  const response = await cache.match('preferiti');
  
  let preferiti = [];
  if (response) {
    preferiti = await response.json();
  }
  
  // Aggiungi il lavoro ai preferiti
  preferiti.push(lavoro);
  
  // Salva i preferiti nella cache
  const newResponse = new Response(JSON.stringify(preferiti), {
    headers: { 'Content-Type': 'application/json' },
  });
  await cache.put('preferiti', newResponse);
}

// Rimuovi un lavoro dai preferiti
async function rimuoviPreferito(idLavoro) {
  const cache = await caches.open('preferiti-cache');
  const response = await cache.match('preferiti');
  
  if (response) {
    const preferiti = await response.json();
    const preferitiFiltrati = preferiti.filter(lavoro => lavoro.id !== idLavoro);
    
    const newResponse = new Response(JSON.stringify(preferitiFiltrati), {
      headers: { 'Content-Type': 'application/json' },
    });
    await cache.put('preferiti', newResponse);
  }
...
// Carica i preferiti dalla Cache API
async function caricaPreferiti() {
  const cache = await caches.open('preferiti-cache');
  const response = await cache.match('preferiti');
...
// Mostra i preferiti nella lista
async function mostraPreferiti() {
  const listaPreferiti = document.getElementById('favorites-list');
  const preferiti = await caricaPreferiti();
  
  listaPreferiti.innerHTML = ''; // Pulisce la lista esistente
  
  if (preferiti.length > 0) {
    preferiti.forEach(lavoro => {
      const item = document.createElement('li');
      item.classList.add('preferito');
      item.innerHTML = `
        <img src="${lavoro.foto}" alt="${lavoro.nome}" class="miniatura">
        <h3>${lavoro.nome}</h3>
        <p>${lavoro.info}</p>
        <button class="rimuovi-preferito" data-id="${lavoro.id}">Rimuovi</button>
      `;
      listaPreferiti.appendChild(item);
    });
    
    // Aggiungi evento per rimuovere preferito
    document.querySelectorAll('.rimuovi-preferito').forEach(button => {
      button.addEventListener('click', async (e) => {
        const idLavoro = e.target.getAttribute('data-id');
        await rimuoviPreferito(idLavoro);
        mostraPreferiti(); // Ricarica la lista dei preferiti
      });
    });
  } else {
    listaPreferiti.innerHTML = '<li>Non hai ancora aggiunto preferiti.</li>';
  }
}
